...
std::string WindowsMessageMap::operator()(DWORD Msg, LPARAM LP, WPARAM WP) const noexcept
{
	...
	const auto MessagesMap = Map.find(Msg);

	std::ostringstream OStringStream;
	if (MessagesMap != Map.end())
	{
		OStringStream << std::left << std::setw(FirstColWidth) << MessagesMap->second << std::right;
	}
	else
	{
		std::ostringstream OStringStreamLocal;
		OStringStreamLocal << "Unknown message: 0x" << std::hex << Msg;
		OStringStream << std::left << std::setw(FirstColWidth) << OStringStreamLocal.str() << std::right;
	}
	OStringStream << "   LP: 0x" << std::hex << std::setfill('0') << std::setw(8) << LP;
	OStringStream << "   WP: 0x" << std::hex << std::setfill('0') << std::setw(8) << WP << std::endl;
